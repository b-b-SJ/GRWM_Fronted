...
export const useTodoApi = (user, isAuthenticated, getAuthHeaders) => {
  ...
  const getTodos = useCallback(
    async (userId, params = {}) => {
      ...
      try {
        ...
        queryParams.append(
          "limit",
          params.limit !== undefined ? params.limit : 100
        ); // 기본값 100
    ...
    async (userId, todoId) => {
      ...
      try {
        const response = await fetch(
          `/api/users/${userId}/todos/${todoId}/complete`,
          {
            method: "PATCH",
            headers: {
              ...getAuthHeaders(),
   ...
   * 2.1 반복 To-Do 목록 조회
   */
  const getRecurringTodos = useCallback(
    async (userId, params = {}) => {
      checkAuth();
      setLoading(true);
      setError(null);
   ...
   * 2.2 반복 To-Do 생성
   */
  const createRecurringTodo = useCallback(
    async (userId, recurringTodoData) => {
      checkAuth();
      setLoading(true);
      setError(null);

      const { recurrenceType, recurrenceConfig, startDate, ...rest } =
        recurringTodoData;
      ...
      if (recurrenceConfig) {
        if (recurrenceType === "daily" && recurrenceConfig.daily) {
          ...
          processedRecurrenceConfig.interval =
            recurrenceConfig.daily.repeatInterval || 1;
        } else if (recurrenceType === "weekly" && recurrenceConfig.weekly) {
          processedRecurrenceConfig.weekly =
            recurrenceConfig.weekly.daysOfWeek || [];
        } else if (recurrenceType === "monthly" && recurrenceConfig.monthly) {
          processedRecurrenceConfig.monthly =
            recurrenceConfig.monthly.dayOfMonth || 0;
        }
      }
  ...
  const updateRecurringTodo = useCallback(
    async (userId, recurringId, recurringTodoData) => {
      checkAuth();
      setLoading(true);
      setError(null);
      ...
      const dataToSend = {
        ...
        repeatRange:
          recurringTodoData.repeatRange || recurringTodoData.recurrenceType,
        // 개별 필드로 전송
      ...
      try {
        const response = await fetch(
          `/api/users/${userId}/recurring-todos/${recurringId}`,
          {
            method: "PUT",
            headers: {
              ...getAuthHeaders(),
   ...
   * 2.4 반복 To-Do 삭제
   */
  const deleteRecurringTodo = useCallback(
    async (userId, recurringId) => {
      checkAuth();
      setLoading(true);
      setError(null);
   ...
   * 2.5 반복 To-Do 자동 생성
   */
  const generateRecurringTodos = useCallback(
    async (userId, params = {}) => {
      checkAuth();
      setLoading(true);
      setError(null);
      ...
      if (
        processedParams.targetDate &&
        processedParams.targetDate instanceof Date
      ) {
        processedParams.targetDate = processedParams.targetDate
          .toISOString()
          .split("T")[0]; // "YYYY-MM-DD"
      }
      try {
        const response = await fetch(
          `/api/users/${userId}/recurring-todos/generate`,
          {
            method: "POST",
            headers: {
              ...getAuthHeaders(),
