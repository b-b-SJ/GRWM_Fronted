...
export const AuthProvider = ({ children }) => {
  ...
  //플래너 관련 저장소 정리
  const clearPlannerStorage = useCallback(() => {
  ...
  const handleOAuthLogin = useCallback(
    async ({ accessToken, userId, username, communityNickname }) => {
  ...
  const refreshFcmToken = async (userId) => {
    ...
    try {
      ...
      if (!fcmToken) {
        console.warn(
          "[FCM Refresh] FCM 토큰 발급 실패. 서버에 업데이트하지 않습니다."
        );
        return;
      }

      console.log(
        `[FCM Refresh] 식별자: ${userId}, FCM Token: ${fcmToken}으로 서버 업데이트 시도.`
      );
      ...
      if (response.ok) {
        ...
        console.error(
          "[FCM Refresh] 서버에 FCM 토큰 저장 실패:",
          response.status
        );
      }
    } catch (error) {
      console.error(
        "[FCM Refresh] FCM 토큰 재발급 및 업데이트 중 오류 발생:",
        error
      );
    }
  };
  ...
  const login = async (loginId, password) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const { tokenType, accessToken, username, userId, communityNickname } =
          data;
